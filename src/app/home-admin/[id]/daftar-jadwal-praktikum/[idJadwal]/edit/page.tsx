...
type EditJadwalPraktikumType = {
  id_modul: string | number;
  start_tgl: string | Date;
  start_wkt: string | Date;
  kuota: number;
};

export default withAuth(EditJadwalPraktikum, ['admin']);

function EditJadwalPraktikum({
  params,
}: {
  params: { id: string; idJadwal: string };
}) {
  const methods = useForm<EditJadwalPraktikumType>({
    mode: 'onTouched',
  });
  ...
  const { mutate: post } = useMutation<
    void,
    AxiosError<ApiError>,
    EditJadwalPraktikumType
  >({
    mutationFn: async (data) => {
      try {
        const res = await api.put(
          `/praktikum/${params.id}/modul/jadwal/${params.idJadwal}`,
          data,
          {
            toastify: true,
          }
        ...
        if (res.data) {
          ...
          alert('Berhasil mengedit jadwal praktikum');
        } else if (!res.data) {
  ...
  const onSubmit: SubmitHandler<EditJadwalPraktikumType> = (data) =>
    post({
                ...
                <div className='flex flex-col gap-1.5'>
                  ...
                  >
                    {dataJadwal?.map((jadwal) => (
                      <option value={jadwal.id_modul} key={jadwal.id_modul}>
                        {jadwal.judul_modul}
                      </option>
                    ))}
                  </select>
