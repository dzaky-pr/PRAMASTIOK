...
export default withAuth(TambahDosen, ['admin']);

function TambahDosen({ params }: { params: { id: string } }) {
  ...
  const methods = useForm<TambahDosen>({
    mode: 'onTouched',
  });
  ...
  >({
    mutationFn: async (data) => {
      const res = await api.post(`/role/${params.id}/dosen`, data, {
        toastify: true,
      });
  ...
  const onSubmit: SubmitHandler<TambahDosen> = (data) => {
    RegistMutation({
